...
import { medicareLegacyQuoteConfig, commonBreeds } from '@/data/quoteConfig';

interface MedicalCareLegacyQuoteCalculatorProps {
  initialTierId: 'complete-care' | 'ultimate-care';
}

const MedicalCareLegacyQuoteCalculator: React.FC<MedicalCareLegacyQuoteCalculatorProps> = ({ initialTierId }) => {
  ...
  const currentReimbursement = medicareLegacyQuoteConfig.reimbursementMapping[selectedTierId];
  const currentAnnualCoverageLimit = medicareLegacyQuoteConfig.annualCoverageLimitMapping[selectedTierId];
  const currentDeductible = medicareLegacyQuoteConfig.deductibleMapping[selectedTierId];
  const currentAccidentalDeathBenefit = medicareLegacyQuoteConfig.accidentalDeathBenefit[selectedTierId];
  const currentBurialCremationAssistance = medicareLegacyQuoteConfig.burialCremationAssistance[selectedTierId];
  ...
  const calculatePremium = () => {
    let premium = medicareLegacyQuoteConfig.basePremiums[selectedTierId];

    // Apply pet type multiplier
    premium *= medicareLegacyQuoteConfig.petTypeMultipliers[petType];

    // Apply age multiplier
    const effectiveAge = Math.min(petAge, Math.max(...Object.keys(medicareLegacyQuoteConfig.ageMultipliers).map(Number)));
    premium *= medicareLegacyQuoteConfig.ageMultipliers[effectiveAge] || 1;

    // Apply breed multiplier
    premium *= medicareLegacyQuoteConfig.breedMultipliers[petBreed] || 1;
  ...
  const tierOptions = [
    { id: 'complete-care', name: 'Tier 1: Complete Care' },
    { id: 'ultimate-care', name: 'Tier 2: Ultimate Care' },
  ];

  return (
    <div className="bg-white p-6 rounded-lg shadow-md mb-8 border border-gray-200">
      <h3 className="text-2xl font-bold text-[#342d47] mb-6 text-center">
        Get Your Medical Care & Legacy Plan Estimate
      </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div>
          ...
          <input
            ...
            id="petAge"
            min="0"
            max="12" // Based on your current age multiplier config for this combined plan
      ...
      <div className="mb-6 bg-gray-50 p-4 rounded-md border border-gray-100">
        <h4 className="text-lg font-semibold text-gray-700 mb-2">Selected Tier Details & Benefits:</h4>
        <p className="text-sm text-gray-600">
            <span className="font-medium">Reimbursement Rate:</span> {currentReimbursement}
        </p>
        <p className="text-sm text-gray-600">
            <span className="font-medium">Annual Coverage Limit:</span> {currentAnnualCoverageLimit}
        </p>
        <p className="text-sm text-gray-600">
...
export default MedicalCareLegacyQuoteCalculator;